...
import { DownloadsBadge, DownloadsPanel, useDownloadCount } from '@shared/index';
    ...
    return (
        <>
            <div className="h-8 w-full flex items-center justify-between px-4 bg-slate-50 dark:bg-[#0b101b] border-b border-slate-200 dark:border-white/5 drag-region sticky top-0 z-[100] active:cursor-grabbing group transition-colors duration-300">
                ...
                <div className="flex items-center gap-3 pr-[120px] no-drag">
                    {/* Downloads badge */}
                    <DownloadsBadge onClick={() => setPanelOpen(v => !v)} count={dlCount} />
                    ...
                    <a
                        ...
                        className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 hover:text-blue-500 dark:hover:text-blue-400 transition-all hover:scale-105 cursor-pointer"
                    >
                        <Globe size={10} /> Web
                    </a>

                    <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-emerald-500/80 select-none cursor-default bg-emerald-500/5 px-2 py-0.5 rounded-full border border-emerald-500/10 hover:bg-emerald-500/10 transition-colors">
            ...
            <DownloadsPanel isOpen={panelOpen} onClose={() => setPanelOpen(false)} />
        </>
    );
};
